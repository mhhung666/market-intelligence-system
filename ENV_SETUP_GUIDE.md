...
`run_daily_workflow.sh` 已更新，支援從 `.env` 檔案讀取環境變數，讓設定更安全、更靈活。
...
`run_daily_workflow.sh` 會按以下順序尋找 token：

1. 檢查 `.env` 檔案中的 `CLAUDE_TOKEN` 或 `CLAUDE_CODE_OAUTH_TOKEN`
2. 如果 `.env` 不存在，檢查 `.env.docker`
3. 如果都找不到，嘗試從 `~/.config/claude/credentials.json` 讀取
4. 如果還是找不到，顯示錯誤並退出
...
📅 日期: 2025-12-08
⏰ 時間: 13:30:00
📂 專案路徑: /Users/mhhung/Development/MH/market-intelligence-system
```
...
在 `run_daily_workflow.sh` 的開頭加入除錯資訊：
...
### Q: .env 和 .env.docker 有什麼不同？

- `.env` - 本地 macOS 使用（方案 A）
- `.env.docker` - Docker 環境使用
- 腳本會優先使用 `.env`，找不到時才用 `.env.docker`
...
會嘗試從以下位置讀取：
1. `.env.docker`
2. `~/.config/claude/credentials.json`（需要 jq）
3. 如果都失敗，顯示錯誤並退出
...
- ✅ `run_daily_workflow.sh` - 已更新，支援 .env
