...
if command -v claude &> /dev/null; then
    ...
    echo -e "${YELLOW}   請手動更新 docs/web/market.html 和 docs/web/holdings.html${NC}"
    exit 0
fi
...
if [[ -n "${LATEST_MARKET}" ]]; then
    ...
    echo -e "   目標: ${DOCS_WEB_DIR}/market.html"

    # 使用 Claude CLI 進行轉換
cat <<EOF | "${CLAUDE_BIN}" > "${DOCS_WEB_DIR}/market.html"
...
1. 使用與現有 ${DOCS_WEB_DIR}/market.html 完全相同的結構和樣式
...
if [[ -n "${LATEST_HOLDINGS}" ]]; then
    ...
    echo -e "   目標: ${DOCS_WEB_DIR}/holdings.html"

    # 使用 Claude CLI 進行轉換
cat <<EOF | "${CLAUDE_BIN}" > "${DOCS_WEB_DIR}/holdings.html"
請將以下 markdown 報告轉換成完整的 HTML 頁面,並嚴格遵循以下格式:
...
1. 使用與現有 ${DOCS_WEB_DIR}/holdings.html 完全相同的結構和樣式
2. 導航欄保持一致 (Home, Market Analysis, Holdings Analysis)
...
if [[ -n "${LATEST_MARKET}" ]] || [[ -n "${LATEST_HOLDINGS}" ]]; then
    ...
    # 使用 sed 更新日期 (如果存在的話)
    if [[ -f "${DOCS_WEB_DIR}/index.html" ]]; then
        # 備份
        cp "${DOCS_WEB_DIR}/index.html" "${DOCS_WEB_DIR}/index.html.bak"

        # 更新日期
        sed -i "s/最後更新: [0-9-]\+/最後更新: ${TODAY}/g" "${DOCS_WEB_DIR}/index.html"
        sed -i "s/<span class=\"date\">[0-9-]\+<\/span>/<span class=\"date\">${TODAY}<\/span>/g" "${DOCS_WEB_DIR}/index.html"
...
echo ""
echo -e "${GREEN}下一步:${NC}"
echo -e "  1. 預覽: cd docs/web && python3 -m http.server 8000"
